          ...
          <div className="w-1/2 max-[1050px]:w-full  justify-center  flex flex-col gap-8 max-md:gap-4 ">
            ...
            <div className="grid grid-cols-2  w-full gap-4  ">
              {contact_links?.map((link, index) => {
                return (
                  <div
                    className={`flex rounded-xl bg-neutral  gap-4 items-center justify-start py-4 px-8 max-[520px]:px-4  ${index === 0 || index + 1 === contact_links.length ? 'col-span-2 ' : 'col-span-1 max-[520px]:col-span-2 max-[1050px]:col-span-2 max-md:col-span-1'}    `}
                    key={link?.identifier}
                  >
                    {/* className=
